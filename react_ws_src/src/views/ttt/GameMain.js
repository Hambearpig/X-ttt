...
export default class SetName extends Component {
  constructor(props) {
    ...
    if (this.props.game_type != "live")
      this.state = {
        ...
        comp_strat: [...rand_arr_elem(this.win_sets)],
      };
    else {
      this.sock_start();
  ...
  turn_comp() {
    ...
    const c = empty_cells_arr.includes(this.state.comp_strat[0])
      ? this.state.comp_strat[0]
      : rand_arr_elem(empty_cells_arr);

    if (empty_cells_arr.includes(this.state.comp_strat[0])) {
      this.state.comp_strat.shift();
    }
